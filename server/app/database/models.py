...
class User(Base, BaseModel):
    ...
    global_role: Mapped[Optional[str]] = mapped_column(sa.Enum('admin', 'support', 'auditor', name='global_role_enum'), nullable=True)
    created_at: Mapped[datetime] = mapped_column(sa.DateTime, nullable=False, server_default=sa.func.now())
    updated_at: Mapped[datetime] = mapped_column(sa.DateTime, nullable=False, server_default=sa.func.now(), onupdate=sa.func.now())
...
class Patient(Base, BaseModel):
    ...
    sex: Mapped[Optional[str]] = mapped_column(sa.Enum("male", "female", "other", name="sex_enum"), nullable=True)
...
class Clinic(Base, BaseModel):
    ...
    is_approved: Mapped[bool] = mapped_column(sa.Boolean, nullable=False, server_default=sa.false())
...
class Doctor(Base, BaseModel):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    user_id: Mapped[int] = mapped_column(sa.ForeignKey('users.id'), nullable=False)
    clinic_id: Mapped[Optional[int]] = mapped_column(sa.ForeignKey('clinics.id'), nullable=True)
...
class ClinicMembership(Base, BaseModel):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    clinic_id: Mapped[int] = mapped_column(sa.ForeignKey('clinics.id', ondelete='CASCADE'), nullable=False)
    user_id: Mapped[int] = mapped_column(sa.ForeignKey('users.id', ondelete='CASCADE'), nullable=False)
    role: Mapped[str] = mapped_column(sa.Enum("doctor", "patient", "admin", "facilitator", name="role_enum"), nullable=False)
...
class Diagnosis(Base, BaseModel):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    patient_id: Mapped[int] = mapped_column(sa.ForeignKey('patients.id'), nullable=False)
...
class Booking(Base, BaseModel):
    ...
    diagnosis_id: Mapped[Optional[int]] = mapped_column(sa.ForeignKey('diagnosis.id'), nullable=True)
    status: Mapped[str] = mapped_column(sa.Enum('cancelled', 'confirmed', 'tentative', name='booking_status_enum'), nullable=False, server_default='tentative')
