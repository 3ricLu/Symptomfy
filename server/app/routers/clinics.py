...
def createClinic(db: Session):
    ...
    try:
        ...
        clinic_admin: ClinicMembership = clinic_crud.add_user_to_clinic(clinic_id=new_clinic.id, user_id=user.id, role="admin")
        if is_doctor:
            clinic_doctor: ClinicMembership = clinic_crud.add_user_to_clinic(clinic_id=new_clinic.id, user_id=user.id, role="doctor")
...
def getPendingClinics(db: Session):
    ...
    return make_response(jsonify([clinic.baseInformation() for clinic in pending_clinics]), 200)
...
def setPendingClinics(db: Session):  
    ...
    try:
        ...
        return make_response(jsonify({"message": f"Successfully approved clinic with id {clinic_id}"}), 200)
    except Exception as e:
