...
def build_auth_response(user_crud: UserCrud, user: User, tokens, message):
    return {
        "message": message,
        "user-role": user_crud.userRole(user=user),
        **tokens
    }
...
def create_user(db) -> Response:
    ...
    return make_response(jsonify(
        build_auth_response(user_crud, user, tokens, f"User with ID {user.id} Successfully Created")
    ), 201)
...
def authenticate_user(db) -> Response:
    ...
    return make_response(jsonify(
        build_auth_response(user_crud, user, tokens, f"User with email {email} Successfully Authenticated")
    ), 200)
...
def refresh_user_token(db) -> Response:
    ...
    if not refresh_token_value:
        return make_response(jsonify({"message": "Missing required header: refresh-token"}), 400)
