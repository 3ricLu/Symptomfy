        ...
        def wrapper(db: Session, *args, **kwargs):
            ...
            if not user_role:
                if "doctor" in allowed_roles:
                    user_role = "doctor" if doctor_crud.isDoctor(user_id=user.id) else None
                elif "patient" in allowed_roles:
                    user_role = "patient" if patient_crud.isPatient(user_id=user.id) else None

            if user_role not in allowed_roles:
                return make_response(
                    jsonify({"message": f"User with id {user.id} does not have required role(s): {allowed_roles}"}), 403
                )
